...
# Create summary and prediction entries
freq_summary <- data.frame(
  study = "Frequentist (Summary Estimate)",
  c_stat = res_sens$est,
  c_stat_LL = res_sens$ci.lb,
  c_stat_UL = res_sens$ci.ub,
  type = "Summary"
)

bayes_summary <- data.frame(
  study = "Bayesian (Summary Estimate)",
  c_stat = res_sens_bayes$est,
  c_stat_LL = res_sens_bayes$ci.lb,
  c_stat_UL = res_sens_bayes$ci.ub,
  type = "Summary"
)
...
freq_pred <- data.frame(
  ...
  c_stat = NA,
  c_stat_LL = res_sens$pi.lb,
  c_stat_UL = res_sens$pi.ub,
  type = "Prediction"
)
...
bayes_pred <- data.frame(
  ...
  c_stat = NA,
  c_stat_LL = res_sens_bayes$pi.lb,
  c_stat_UL = res_sens_bayes$pi.ub,
  type = "Prediction"
)
